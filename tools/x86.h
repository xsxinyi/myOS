...
// 指令LTR（加载任务寄存器）和STR（保存任务寄存器）加载和保存任务寄存器的可见部分。LTR指令让任务寄存器加载TSS描述符的段选择符，该指令只能运行在特权级0，
// 该指令通常用来系统初始化时初始化任务寄存器。装入到TR的选择子不能为空，必须索引位于GDT中的描述符，且描述符的类型必须是TSS。
// 指令STR可以将任务寄存器的可见部分保存到通用寄存器或内存中。该指令可以运行在任何特权级。
//   TSS描述符仅可能存放在GDT中，不能存放在LDT或IDT中。
// ————————————————
// 版权声明：本文为CSDN博主「嗷大墨」的原创文章，遵循CC 4.0 BY-SA版权协议，转载请附上原文出处链接及本声明。
// 原文链接：https://blog.csdn.net/Gyc8787/article/details/121779971
static inline void
ltr(uint16_t sel) {
